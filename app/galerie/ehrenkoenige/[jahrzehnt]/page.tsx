...
export async function generateMetadata({ params }: Props) {
  return {
    title: `Ehrenkönig:innen ${params.jahrzehnt}`,
    description: `Bilder der Ehrenkönig:innen aus den ${params.jahrzehnt} Jahren.`,
  };
}
...
export default async function JahrzehntPage({ params }: Props) {
  ...
  const allSubfolders = await getSubfolders("ehrenkoenig-gallery");
  ...
  const resolved = "ehrenkoenige-gallery";
    ...
    <>
      <section className="page-hero">
        <div className="container">
          ...
          <h1>Ehrenkönig:innen {params.jahrzehnt}</h1>
          <p>
      ...
      <section className="section" style={{ paddingTop: "4.5rem", paddingBottom: "4.5rem" }}>
        <div className="container">
          <Link href="/galerie/ehrenkoenige" className={styles.backLink}>
            ← Alle Jahrzehnte
          </Link>
